...
def create_bev_writer(output_dir, scene_name, map_w, map_h, fps=30):
    """
    Create BEV video writer with map dimensions
    """
    bev_dst = f"{output_dir}/{scene_name}_bev.mp4"
    bev_writer = cv2.VideoWriter(bev_dst, cv2.VideoWriter_fourcc(*'mp4v'), fps, (map_w, map_h))
    print(f"[BEV] Created video writer: {bev_dst} ({map_w}x{map_h})")
    return bev_writer
...
def write_vids_3d_with_bev(trackers, imgs, src_handlers, pose, _COLORS, mc_tracker, cur_frame, 
                          ...
                          tx=None, ty=None, map_h=None, bev_writer=None):
    """
    Modified version of write_vids that supports 2D, 3D, and BEV visualization
    """
    outputs = []
    ...
    # Initialize BEV map for this frame
    bev_map = None
    if map_img is not None:
        bev_map = map_img.copy()
                ...
                if track.is_activated:
                    ...
                    try:
                        ...
                        if corners_3d_world is not None:
                            ...
                            # Project to BEV if map is available
                            if bev_map is not None and scale_factor and tx is not None and ty is not None and map_h:
                                bev_map = project_3d_box_to_bev(
                                    corners_3d_world, bev_map, scale_factor, tx, ty, map_h,
                                    color=color, thickness=2)
                                
                                # Add track ID on BEV map
                                if center_3d_world is not None:
                                    ...
                                    if 0 <= map_x < bev_map.shape[1] and 0 <= map_y < bev_map.shape[0]:
                                        cv2.putText(bev_map, f'{track_id}', (map_x, map_y), 
    ...
    # Write BEV frame
    if bev_writer is not None and bev_map is not None:
        # Add frame number and timestamp to BEV
        cv2.putText(bev_map, f'Frame: {cur_frame}', (10, 30), 
                   cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
        bev_writer.write(bev_map)
    
    return outputs


def finalize_cameras_with_bev(src_handlers, bev_writer=None):
    """
    Release video readers and writers including BEV writer
    """
...
def run(args, conf_thres, iou_thres, data_root, scene, enable_3d=False, log_file=None):
    ...
    if enable_bev and enable_3d:
        ...
        if map_img is not None:
            ...
            os.makedirs(output_dir, exist_ok=True)
            bev_writer = create_bev_writer(output_dir, scene, map_w, map_h, fps=30)
    ...
    # Cleanup
    finalize_cameras_with_bev(camera_handlers, bev_writer)
    ...
    if enable_bev and map_img is not None:
        print(f'[BEV] Bird\'s Eye View video saved as: output_videos/{scene}/{scene}_bev.mp4')
...
if __name__ == '__main__':
    ...
    print(f"BEV mode: {'Enabled' if enable_3d else 'Disabled'} (automatically enabled with 3D)")
