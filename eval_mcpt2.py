...
# Initialize logging
log_file = f"tracking_log_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
sys.stdout = TerminalLogger(log_file)
print(f"=== Multi-Camera Tracking Started ===")
...
def get_video_readers_and_writers(data_root, scene_name, enable_3d=False):
    """
...
def write_vids_3d(trackers, imgs, src_handlers, pose, _COLORS, mc_tracker, cur_frame, depth_maps=None, calibrations=None):
    """
...
def run(args, conf_thres, iou_thres, data_root, scene, enable_3d=False):
    ...
    print(f"Number of perspective transforms: {len(perspective_transforms) if perspective_transforms else 0}")
    ...
    while cur_frame <= total_frames:
        ...
        # Debug ReID every 10 frames
        if cur_frame % 10 == 1:
            print(f"\n=== ReID Debug Frame {cur_frame} ===")
            for cam_idx, tracker in enumerate(trackers):
                reid_tracks = sum(1 for t in tracker.tracked_stracks if t.curr_feat is not None)
                pose_tracks = sum(1 for t in tracker.tracked_stracks if hasattr(t, 'pose') and t.pose is not None)
                print(f"Camera {cam_idx}: {len(tracker.tracked_stracks)} tracks, {reid_tracks} with ReID, {pose_tracks} with pose")
                
                # Show first few track details
                for i, track in enumerate(tracker.tracked_stracks[:3]):
                    feat_status = "✓" if track.curr_feat is not None else "✗"
                    smooth_status = "✓" if track.smooth_feat is not None else "✗"
                    global_id = getattr(track, 'global_id', 'None')
                    print(f"  Track {track.track_id} (global: {global_id}): feat={feat_status}, smooth={smooth_status}")
            
            # Multi-camera association status
            print(f"Multi-camera tracker: {len(mc_tracker.tracked_mtracks)} global tracks")
        ...
        if args['write_vid']:
            if enable_3d:
                ...
                for i, (tracker, img, (_, writers, camera_name, _)) in enumerate(zip(trackers, imgs, src_handlers)):
